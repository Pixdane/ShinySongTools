...
use scsp_core::{CallbackObservability, CompactEvent, CompactEventCode, CompactLevel};
...
fn spawn_drain_worker(dispatch: Dispatch) {
    let _ = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let queue = scsp_core::process_event_queue();
            loop {
...
/// events via `scsp_core::process_event_queue()` instead of tracing.
#[must_use]
...
mod tests {
    use super::*;
    use scsp_core::{CompactOwnerId, CompactSiteId};
