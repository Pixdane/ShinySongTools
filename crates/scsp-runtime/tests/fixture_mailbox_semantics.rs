...
use scsp_core::{DataRoot, RuntimeGate};
...
impl HookTarget for FixtureTarget {
    const TARGET: scsp_core::TargetId = scsp_core::TargetId {
...
scsp_plugin_api::define_hook_site!(MAILBOX_SITE: HookSite<FixtureTarget, MailboxSites>);
        ...
        *self.sites_out.lock().expect("sites out") = Some(Arc::clone(&sites));
        ...
        let _installed = match published {
            ...
            Err(err) => {
                ...
                return Err(scsp_plugin_api::PluginError::Hook(err));
            }
        };
