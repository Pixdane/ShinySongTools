...
impl App {
    ...
    pub fn set_method_resolver(&mut self, resolver: Arc<dyn corelib::MethodResolver>) {
    ...
    fn build_rollback(&mut self, owner: OwnerId, err: &corelib::PluginError) {
...
fn execute_restore(action: RestoreAction, main: Option<&MainThreadToken>) {
    let result = match action {
        RestoreAction::AnyThread(f) => match catch_unwind(AssertUnwindSafe(f)) {
            Ok(r) => r,
            Err(_) => Err(corelib::RestoreError::Failed),
        },
